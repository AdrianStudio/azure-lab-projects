...
D) Many people would choose this option, I did too at one point, but its not the best option. THis may seem correct since the password is not hardcoded anywhere, and it doesnt have to be rotated every X period. But it has a loop issue. It uses MI to acces Key Vault, to then retrieve it, it does the same as option C, but 5 times more complicated. This option would be good but it also defeats the purpose. And the key can also get leaked, if anyone writes it down, sends it via chat or access it. 

Veredict:

Option C is better, not only security and performace wise. It meets clients expectations, and thats always the offer you want to give.
